...
import { getBlogPosts, saveBlogPosts, staticBlogPosts, type BlogPost, type AffiliateProduct } from '@/app/lib/blogData';
import { getAnimalTypes, saveAnimalTypes, type AnimalType } from '@/app/lib/blogData';
...
const editAffiliateProduct = (index: number) => {
  ...
  if (product) {
    setCurrentAffiliateProduct({
      ...
      url: product.url ?? '',
      buttonText: product.buttonText ?? 'Jetzt kaufen',
      buttonColor: product.buttonColor ?? 'green',
      buttonStyle: product.buttonStyle ?? 'solid',
      buttonSize: product.buttonSize ?? 'medium',
      buttonIcon: product.buttonIcon ?? 'shopping-cart',
      buttonAnimation: product.buttonAnimation ?? 'hover-scale',
    });
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          ...
                          {staticBlogPosts.some(staticPost => String(staticPost.id) === String(post.id)) && (
